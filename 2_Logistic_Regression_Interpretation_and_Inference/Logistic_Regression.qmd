...
-   briefly explore the data set (the full case study is presented in @desbois2008)
...
-   perform simple Hold-out approach (train/test split) and evaluate ROC / AUC for some `glm()` model specification of your choice (ie choose manually a subset of variables)

We choose to split using a 70%/30% repartition between training and testing set. Here the data set is balanced and train/test using a simple random sampling have a similar distribution:
...
# As in the Desbois article, the AUC figures are around 0.96 which is quite elevated!
...
Splitting the data set into 10 flods or blocks:
...
It is a less efficient and lengthy code than before, but it is also easier to understand and adapts to your needs (we commented the code below as it takes time to run, launching five times a 10-fold Cross Validation for 10 different models, alternatively you can diminish the repeat and fold number):
