...
Manual likelihood-ratio-test-based forward selection (as described in Desbois)
...
second_step
```
`r21` is the most significant variable in the second step (it differs from the article where r32 is selected (p70, the LRT is the same for r32 (129.65) but r21 and r14 have higher LRT))
...
Manual likelihood-ratio-test-based forward selection with override (ie selecting the same variables as Desbois if needed)
...
In the article `r32` is the most significant variable in the second step: we choose `r32`instead of `r21`. 
...
sixth_step
```
Like in the article `r12` is the most significant variable in the sixth step (with same LRT)


R performs automated AIC/BIC based vairable selections using (step)
...
(don_desbois_binned <- don_desbois %>%
    mutate(r17_bins = cut(r17, breaks = seq(0, 0.15, class_width),
                              right = FALSE, dig.lab = 4),
