...
logit_risk <- logit_error_rate %>% summarise(l01 = mean(l01))
```
...
logit_splines_add_error_rate <- logit_splines_add_error_rate  %>%
    summarise(l01 = mean(l01)) %>% 
    pull(l01)
```

The empirical risk on testing set is `r round(logit_splines_add_error_rate, 3)`.
...
logit_splines_add_tens_error_rate <- logit_splines_add_tens_error_rate  %>%
    summarise(l01 = mean(l01)) %>% 
    pull(l01)

```

The empirical risk on testing set is `r round(logit_splines_add_tens_error_rate, 3)`.
...
logit_binned_error_rate <- logit_binned_error_rate  %>%
    summarise(l01 = mean(l01)) %>% 
    pull(l01)
```

The empirical risk on testing set is `r round(logit_binned_error_rate, 3)`.
...
```{r}
# TODO Compare ROC curves of classifiers
# ROC Curve
library("ROCR")    
...
# 
# # CART
# pred <- prediction(cart_error_rate$prob[,"1"] , cart_error_rate$Y)    
# perf <- performance(pred, measure = "tpr", x.measure = "fpr") 
# plot(perf, col=rainbow(7), add = TRUE, main="ROC curve Admissions", xlab="Specificity", 
#      ylab="Sensitivity")  
...
$\alpha(s)$ is called **false positive** rate and $\beta(s)$ **false negative** rate. Similarly **specificity** and **sensibility** are defined:
...
\vdots  & \ddots & \vdots \\
\cdots  & \cdots & p_\beta(x_1n(1-p_\beta(x_n))
\end{pmatrix}
\end{align} 
$$
