...
[^3]: Here I use the same terminology as in Sébastien Gadat course which is very clear;

-   the "generative" or "probabilistic" approach: we model the joint distribution $\mathbf P_{\mathrm X \times \mathrm Y}$, we estimate the regression function $\eta(x)$ and use this estimate "inside" the Bayes classifier;
